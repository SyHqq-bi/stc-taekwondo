...
import { Mail, KeyRound, ArrowLeft, Flame, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function ForgotPassword() {
  ...
  const handleResetPassword = async (e) => {
    ...
    // Kirim email reset password dari Supabase
    ...
    if (error) {
      ...
      setSuccessMsg('Link pemulihan password telah dikirim ke email kamu! Silakan periksa kotak masuk atau folder spam emailmu.');
    }
    setLoading(false);
  };
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-8 shadow-xl">
          <div className="text-center mb-8">
            <span className="inline-flex items-center gap-1.5 text-[10px] font-black tracking-[0.2em] text-red-600 uppercase bg-red-50 px-3.5 py-1 rounded-full border border-red-100">
              <Flame size={12} className="text-red-600 fill-red-600 animate-pulse" /> PEMULIHAN AKUN
            </span>
            <h1 className="text-2xl font-black text-slate-950 font-heading uppercase mt-3 tracking-tight">
              Lupa <span className="text-red-600">Kata Sandi?</span>
            </h1>
            <p className="text-xs text-slate-500 mt-1 font-medium">Masukkan email terdaftar kamu untuk menerima link reset password.</p>
          </div>
          ...
          {successMsg && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold rounded-2xl flex items-center gap-2 leading-relaxed">
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 text-center">
            <Link to="/login" className="inline-flex items-center gap-2 font-extrabold text-xs text-slate-600 hover:text-red-600 uppercase tracking-wider transition">
